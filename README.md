...
This fork uses the authenticated H3 transport from the separate [Tailcat transport dependency](https://github.com/LiuTangLei/tailcat-tailscale) and [Tailcat QUIC dependency](https://github.com/LiuTangLei/tailcat-quic-go). **TCP proxy connections use reliable HTTP/3 CONNECT streams**, avoiding a second user-space TCP stack inside QUIC. UDP/IP datagrams use HTTP/3 CONNECT-IP and QUIC DATAGRAM on the same authenticated H3 connection. Neither path wraps WireGuard ciphertext. **BBRv3 is selected by the application on both QUIC endpoints.** Changing the operating system's TCP congestion-control setting does not select the QUIC controller.
